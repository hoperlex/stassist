/**
 * astro_calendar — предрасчёт лунного календаря (см. docs/architecture/22-модель-данных.md §5,
 * docs/roadmap/prompts/f3-калькуляторы-и-карта.md требование 3). Заполняется ДВУМЯ путями:
 * worker'ом (apps/worker/src/astro-calendar) ежесуточно, скользящее окно 18 месяцев ВПЕРЁД от
 * «сегодня» — И ленивой генерацией «при заходе» в apps/api (apps/api/src/lunar-calendar/
 * lazy-generate.ts, находка [lunnyj-kalendar-empty]) — она же покрывает прошлые месяцы/дни,
 * которые cron в принципе никогда не считает (окно только вперёд).
 *
